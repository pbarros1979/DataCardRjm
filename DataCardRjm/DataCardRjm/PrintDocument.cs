...
namespace DataCardRjm
{
    ...
    internal class SamplePrintDocument : PrintDocument {
        ...
        public void DrawCardFront(PrintPageEventArgs pageEventArgs) {
			using (Font font10 = new Font("Arial", 10))
			using (Font font13 = new Font("Arial", 13))
            using (SolidBrush brush = new SolidBrush(Color.Black))
            using (Pen pen = new Pen(Color.Black)) {
				if (!string.IsNullOrEmpty(_commandLineOptions.nomeCartao))
				{
					pageEventArgs.Graphics.DrawString(_commandLineOptions.nomeCartao, font13, brush, 10, 120);
				}
				if (!string.IsNullOrEmpty(_commandLineOptions.cpfCartao))
				{
					pageEventArgs.Graphics.DrawString(_commandLineOptions.cpfCartao, font10, brush, 10, 140);
				}
					
			}
			using (Font font10 = new Font("Arial", 8))
			using (SolidBrush brush = new SolidBrush(Color.Black))
			using (Pen pen = new Pen(Color.Black))
			{
				if (!string.IsNullOrEmpty(_commandLineOptions.lojaCliente))
				{
					pageEventArgs.Graphics.DrawString(_commandLineOptions.lojaCliente, font10, brush, 190, 165);
				}
				if (!string.IsNullOrEmpty(_commandLineOptions.dataCliente))
				{
					pageEventArgs.Graphics.DrawString("CLIENTE DESDE " + _commandLineOptions.dataCliente, font10, brush, 190, 180);
				}
					
			}
			if (!string.IsNullOrEmpty(_commandLineOptions.codBarraCartao))
			{
				Image img = Code128Rendering.MakeBarcodeImage(_commandLineOptions.codBarraCartao, 6, true);
				pageEventArgs.Graphics.DrawImage(img, 10, 160.0F, img.Width / _sizeFactor, img.Height / _sizeFactor);
			}
